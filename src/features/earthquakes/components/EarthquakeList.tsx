...
export function EarthquakeList() {
  ...
  if (!location) {
    return <p className="text-sm text-slate-500">{t.earthquakes.selectLocation}</p>
  }
              ...
              <div className="flex items-center gap-2">
                ...
                {q.tsunami ? (
                  <span className="inline-flex items-center gap-1 rounded-full bg-rose-50 px-2 py-0.5 text-[10px] font-bold text-rose-600 dark:bg-rose-500/10 dark:text-rose-300">
                    <Waves size={10} /> {t.earthquakes.tsunami}
                  </span>
                ) : null}
              </div>
              <div className="mt-0.5 flex items-center gap-3 text-xs text-slate-500">
                <span>{formatRelativeTime(q.time)}</span>
                <span>{t.earthquakes.depth(q.depth.toFixed(0))}</span>
                <span>{t.earthquakes.distance(q.distanceKm.toFixed(0))}</span>
              </div>
            </div>
