...
export function RecentLocationsRow() {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      <span className="text-xs font-medium uppercase tracking-wide text-slate-400">{t.recent.title}</span>
      {recent.map((loc) => (
        <button
          ...
          className={cn(
            'max-w-[10rem] truncate rounded-full border border-black/5 bg-white px-3 py-1.5 text-xs font-medium text-slate-700 shadow-sm transition',
